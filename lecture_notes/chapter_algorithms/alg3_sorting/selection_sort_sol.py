...
# Problem: this trashes the original list! 
...
scores = [85, 90, 78, 92, 88]
print('scores before:', scores)
sorted_scores = selection_sort2(scores)
print('sorted_scores:', sorted_scores) # should be [78, 85, 88, 90, 92]
print(' scores after:', scores)
...
def selection_sort2(unsorted):
    ...
    n = len(unsorted_copy)
    for i in range(n):
        smallest = min(unsorted_copy)
        sorted.append(smallest)
        unsorted_copy.remove(smallest)
    return sorted

# # alternative implementation: pass a copy of the input list to selection_sort1
# def selection_sort2(unsorted):
#     unsorted_copy = unsorted[:]
#     return selection_sort1(unsorted_copy)

# import sortcheck
# sortcheck.test(selection_sort2)
...
outfile = open('sort_results.csv', 'w')
for n in range(10):
    n = n * 1000
    lst = list(range(n)) # lst = [0, 1, 2, ..., n-1]
    random.shuffle(lst)
    ...
    outfile.write(f'{n}, {elapsed_time}\n')

outfile.close()
print('Experiment complete!')
print('See sort_results.csv for results.')
